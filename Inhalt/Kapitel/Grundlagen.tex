...
\subsection{Begriffsdefinitionen}%grundlege Datenankbegriffe
...
\end{description}
Die Definition dieses Begriffes ist für Kapitel \ref{nosql} notwendig.
...
R-Bäume sind balancierte Bäume und nach \cite[S. 523]{book:kudrass} organisieren sie k-dimensionale Rechtecke mithilfe überlappender Blockregionen.
...
Förderierte Datenbanksysteme (FDBS) sind ebenfalls geografisch verteilte Shated nothing systeme, wobei die beteiligten lokalen DBMS eine höhere Autonomie aufweisen, d.h. dass jeweils eine eigene lokale Datenbank mit lokalem DB-schema vorliegt.\footnote{\cite[S.398]{book:kudrass}}
\end{quote}
...
Nach \cite[S.296]{book:kudrass} wird bei Sharding von Datenbanken eine Relation in disjunkte Partitionen aufgeteilt, die auf verschiedenen Platten gespeichert werden.
Auf der darauf folgenden Seite nennt Kudraß folgende Vorteile:
\begin{itemize}
\item Anfrageoptimierung durch Auslastung der Partitionen.
\item Vereinfachung der Administration der Partitionen.
\item Paralelle Verarbeitung.
\end{itemize}
...
NoSQL steht für eine Bewegung des vergangenen Jahrzehnts, in welcher die Abkehr von klassischen relationalen Systemen gefordert oder zumindest ein Umdenken bestehender Strukturen, Vorgehen und Grundsätze angestrebt wird.
...
Einen Überblick der bestehenden Systeme stellt Herr Edlich unter \url{http://nosql-database.org/} bereit.
...
GeoMesa ist eine unter Apache License Version 2.0 stehende geografische Datenbank der Firma LocationTech mit den Möglichkeiten der verteilten Verarbeitung und Versionierung von geografischen Daten.
