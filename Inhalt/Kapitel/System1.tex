...
Abhängig von der Verteilung der Daten und deren Aufruf ergeben sich unterschiedliche Ergebnisse der Leistungsmessung.
%Nach Kudraß
Kosten=IO Kosten (lesende und schreibende Speicherseiten) + Berechnungskosten (Prüfungen, arithmetische operationen) + Kommunikationskosten    S.260
Kostenmaß ist ohne Einheit
Erwartung der Treffer (Selektivität) und Histogramme (erzeugt mit analyze) sollen einbezogen werden S.261f.
Kosten nach S.300f. : Rechenzeit (syntaxprüfer, BErechnung Ausführungsplanes, daten werden verglichen kombiniert sortiert, zusammenfassung in logischen tabellen), IO Zeit (Anzahl der Aufrufe an Speichersystem und Protokollierung mit internem speichermanagement), Datenübertragungszeit (Netzwerk)
Einflussfaktoren: logisches DB Design, Anwendung, Abfrage, Ausführungsplan, Datenmenge der Abfrage, physisches DB Design, größe der DB, DBMS, Systemlast, Netzwerk, Parallelität, OS
...
Der Lasttest zum messen der Verarbeitungsleistung ruft die SQL Funktion \textit{nutrients.createcontourappl(text, text, text, text)} auf.
