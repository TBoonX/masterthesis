...
Für das Verständnis werden grundlegende Linux Kenntnisse und ein root Zugang auf einem Linux System voraus gesetzt.
...
Dabei wird ein Linux Betriebssystem, 155MB freier Festplattenspeicher für die Übersetzung, Installation und Erstellung eines leeren Datenbankclusters sowie eine Menge von Paketen genannt.
Diese Pakete sind: GNU make, gcc, tar und zlib als benötigte sowie libperl oder libpython als optionale Pakete.
Zusätzlich werden für die Erzeugung der Dokumentation oder der Übersetzung des Quellcodes weitere Pakete benötigt.
Diese Anforderungen setzen eine Standardinstallation eines aktuellen Linux voraus.
...
Um das Kommandozeilentool \verb+pgxc_ctl+ nutzen zu können, muss im Quellcodeordner in \verb+./contrib/pgxc_ctl+ gewechselt und dort \verb+make+ sowie \verb+make install+ ausgeführt werden. Damit wird das Tool übersetzt und in den in der vorangegangenen Installation festgelegten Ordner kopiert.

\subsubsection{Einrichtung}
Postgres-XL ist für den Einsatz als Cluster konzipiert.
So muss die Installation für jeden Knoten vorgenommen werden.
Die Einrichtung der einzelnen Knoten variiert je nach Art des Knotens, wobei ein Knoten entweder eine GTM Instanz oder mehrere Coordinator sowie DataNode Instanzen mit einer GTM-Proxy Instanz enthält.
...
Der Zugriff auf Daten eines Postgres-XL Clusters erfolgt über die Coordinator.
...
So wird im Testsystem der erste Coordinator mit folgendem Aufruf angesprochen:\begin{lstlisting}[language=sql,caption={Verbindung zu erstem Coordinator herstellen},captionpos=b]
...
\begin{lstlisting}[language=sql,caption={SQL Query zum erzeugen einer Tabelle in Postgres-XL},label=lst:createtable,captionpos=b]
Create Table name (serial id, text data) Distribute by Hash(id);
\end{lstlisting}%
...
\begin{lstlisting}[language=sql,caption={SQL Query zum hinzufügen eines aktiven Knoten in einen Postgres-XL Cluster},label=lst:createnode,captionpos=b]
...
\begin{lstlisting}[language=sql,caption={SQL Query  zur Aktualisierung der Cluster Zusammensetzung},label=lst:pgxcreload,captionpos=b]
Select * From pgxc_pool_reload();
\end{lstlisting}%
Weitere Abweichungen sind der Dokumentation zu entnehmen.\footnote{Dokumentation der SQL Befehle: \url{http://files.postgres-xl.org/documentation/sql-commands.html}}
...
Die Daten können auch analog des Schema übertragen werden, dies dauert jedoch auf Grund der Umwandlung von zu Textformaten länger.
...
Gegen eine Ersetzung spricht, dass Trigger in der aktuellen Postgres-XL Version nicht verwendet werden können.
Zwar können Trigger in Funktionen ausgelagert und bei Verwendung der Tabellen aufgerufen werden, aber dies würde einen erheblichen Implementierungsaufwand bedeuten und nicht zu 100\%{} die bestehende Funktionalitäten abbilden.
%Teilintegration
...
Die Kriterien wurden in Kapitel \ref{qualitätsmetriken} definiert, somit werden hier die Randbedingungen, Eingaben, Durchführungen und Ergebnisse dargelegt.
...
Dafür steht ein IBM Rack Server x3850 M2\footnote{IBM x3850 M2: \url{http://www.redbooks.ibm.com/redpapers/pdfs/redp4362.pdf}} mit folgender Ausstattung zur Verfügung:
...
Der Postgres-XL Cluster wird mit Virtualisierung der einzelnen Knoten realisiert.
Als Virtualisierungssoftware kommt \Gls{esxi} in der kostenlosen Version 6.0 mit \Gls{vsphere} 6.0\footnote{VMware vSphere: \url{https://www.vmware.com/de/products/vsphere}} als Testversion zum Einsatz.
...
Auf einem Zabbix Server werden Daten zentral von beliebigen Zabbix Clients empfangen, wobei die Art der zu überwachenden Daten beliebig ist.
Standardmäßig wird die Systemauslastung und allgemeine Daten zum jeweiligen System an den Server gesendet.
...
Im Abschnitt Prototypische Implementierung \ref{grundlagen-funktionstests} wurden die Funktionstests bereits als Black-Box Tests definiert und der Umfang im Kapitel \ref{subsection:testfaelle} genauer festgesetzt.
...
Die Priorität hängt vom geschätzten Implementierungsaufwand zur Behebung des Fehlschlages des Testfalles ab.
Der Begriff des Schlages wird nachfolgend verwendet und meint ein bewirtschaftes Flurstück.
...
\textbf{FT04:}\\
Mit PostGIS kann mit der Funktion st\_{}transform direkt zwischen den Koordinatenreferenzsystemen umgerechnet werden, siehe Listing \ref{lst:sttransform}.
...
\begin{lstlisting}[language=sql,caption={SQL Query zur Umrechnung zwischen Koordinatenreferenzsystemen},label=lst:sttransform,captionpos=b]
...
\begin{lstlisting}[language=bash,caption={PostGIS Funktionen der Verschneidung},label=lst:fctsinsection,captionpos=b]
...
Die Geometrien der Schläge 1 und 2 wurden getauscht und beide Ergebnisse dargestellt.
\begin{figure}[h!]
\centering
...
\end{figure}
In allen Fällen sind die Ergebnisse korrekt und die Funktionalität der Verschneidung ist damit für die untersuchten Fälle bewiesen

\textbf{FT06:}\\
Dies wird im zweiten Szenario der Lasttests im nachfolgenden Abschnitt durchgeführt.
%auf schwierigkeiten hinweisen
Da das Ergebnis jedoch nicht überprüft werden kann, gilt der Test als nicht bestanden.
...
\begin{lstlisting}[language=bash,caption={Begrenzendes Rechteck im WKT Format},label=lst:koordinaten,captionpos=b]
...
Da Postgres-XL internal als Transition Typ nicht unterstützt, ist die Funktionen zur Bildung der Vereinigung mit einem Raster nicht verfügbar\footnote{Die Installation von PostGIS wirft Fehler und legt die entsprechenden Funktionen nicht an.}.
...
Zwar ist bei Postgres-XL die Verarbeitung lokaler Anfragen mit mehr administrativen Aufwand verbunden,  wird somit länger gegenüber einer PostgreSQL Instanz bearbeitet, jedoch stehen mehrere Zugriffspunkte auf exklusiven Hardwaresystemen zur Verfügung.
...
Analog des Testaufbaus der Bachelorarbeit des Autors\footnote{Bachelorarbeit des Autors: \cite{ba:kurt}} wird zur Generierung der Last das freie Java Programm JMeter verwendet, welches mehrere vorbereitete Anfragen parallel und verschränkt erzeugt sowie eine Auswertung der Laufzeiten zur Verfügung stellt.
Ein Testlauf wird dabei per Kommandozeile gestartet und die Ergebnisse in einer .csv Datei festgehalten.
...
Dieses Vorgehen orientiert sich an der Masterarbeit von Baas Kapitel 6.1 Objective measurements \cite[S.51]{ma:neo4j}.
...
Nach Kudraß setzen sich die Kosten für die Verarbeitung einer Anfrage aus folgendem zusammen:
\begin{description}
...
\item[Datenübertragung] Meint die Übertragung von Befehlen und Daten zwischen Komponenten. (vgl. \cite[S.300 f.]{book:kudrass})
\end{description}
Weiterhin zählt folgendes zu den Einflussfaktoren:
Das logische Datenbank Design, die Anwendung, die Abfrage, der Ausführungsplan, die Datenmenge der Abfrage, das physische Datenbank Design, die Größe der Datenbank, das \Gls{dbms}, die Systemlast, das Netzwerk, die Parallelität und das Betriebssystem.
...
Geeignet meint hier, dass die Unterabfragen bereits wesentlich die Daten filtern, damit der Coordinator einzig die Untermengen zu einer Menge zusammenfassen und nicht mehr als notwendig Datenmengen aggregiert und diese selbst verknüpfen sowie filtern muss.
...
Für den produktiven Einsatz bei der Agri~Con entsprechend des Anwendungsfalles sind weitere Änderungen in der Verteilung der Daten durchzuführen.
...
Diese Bewertung der Skalierung wird auch auf verteilt arbeitende Systeme angewandt.
...
Die Vorgehensweise stammt von der Postgres-XL Dokumentation.
\begin{description}
\item[Coordinator] Eine Verbindung mit einem anderen Coordinator herstellen und darin Listing \ref{lst:removecoordinator} ausführen.
\item[DataNode] Mit beliebiger Coordinator Instanz verbinden und für jede Relation, welche auf die zu löschende DataNode verteilt ist, \verb+ALTER TABLE tablename DELETE NODE (datanodename);+ ausführen. Betroffene Relationen können mit Listings \ref{lst:getdatanodes} ermittelt werden. Das Tool \verb+pgxc_ctl+ starten und darin \verb+stop datanode datanodename+ ausführen. Auf jedem Coordinator Listing \ref{lst:removedatanode} ausführen.
\end{description}
\begin{lstlisting}[language=sql,caption={SQL Query zur Entfernung eines Coordinator},label=lst:removecoordinator,captionpos=b]
DROP NODE coordinatorname;
SELECT pgxc_pool_reload();
\end{lstlisting}
\begin{lstlisting}[language=sql,caption={SQL Query zur Ermittlung betroffener DataNodes ausgewählter Relationen},label=lst:getdatanodes,captionpos=b]
SELECTc.pcrelid FROM pgxc_class c, pgxc_node n WHERE n.node_name ='datanodename' AND n.oid = ANY (c.nodeoids);
\end{lstlisting}
\begin{lstlisting}[language=sql,caption={SQL Query zur Entfernung eines DataNode},label=lst:removedatanode,captionpos=b]
DROP NODE datanodename;
SELECT pgxc_pool_reload();
\end{lstlisting}

\subsection{Aggregation}
Der erste Lasttest misst die Laufzeit der Aggregation von Punktdaten aus der Tabelle nsensorlogs des Schemas n.
Die Abfrage und Messung erfolgt mit den SQL Statements des Listings \ref{lst:sqlaggregate}.
\begin{lstlisting}[language=sql,caption={SQL Query der Aggregation},label=lst:sqlaggregate,captionpos=b]
Select * From n.nsensorlogs Where fileid=%;
Explain Verbose Select * From n.nsensorlogs Where fileid=%;
\end{lstlisting}
Die erste Query liefert die Daten sowie Laufzeit\footnote{Die Laufzeit wird angezeigt, sofern die Erfassung mit \textbackslash{}timing eingeschaltet wurde.} und die zweite die Aufteilung des SQL Statements an die Knoten des Clusters.
Der Aufruf der ersten Query erfolgt parallel an alle Coordinator Instanzen und wird für den Lasttest verwendet.
...
44982 & 20.494 & 2 & 4 \\ \hline	%dn 4 node2
\end{tabular}
...
Fünf der sechs fileids\footnote{Jeder Coordinator verwendet andere fünf fileids, sodass alle fünf verwendet werden.} werden für die beschriebene Anfrage verwendet und die Ergebnisse in den Arbeitsspeicher geladen.
...
\begin{lstlisting}[language=sql,caption={Explain Verbose Ergebnis eines Coordinator},captionpos=b]
Remote Subquery Scan on all (datanode4)
  -> Index Scan using idx_fileid on n.nsensorlogs
    Index Cond: (nsensorlogs.fileid = 44982)}
\end{lstlisting}
Das Ergebnis ist auf allen Coordinatorn gleich.\footnote{Die Tabelle wurde mit den SQL Befehlen \textit{vacuum} und \textit{analyze} vorbereitet.}
...
Um einen höheren Durchsatz zu erreichen und das Postgres-XL Cluster höher auszulasten, wurden der UMN \Gls{vm} zwei weitere Prozessorkerne zugeordnet.
...
Es wurde dafür die GTM \Gls{vm} verwendet, da sie auch bei Benutzung des Postgres-XL Clusters eine geringe CPU Auslastung aufwies\footnote{Geringe CPU Auslastung der GTM \Gls{vm}: Abbildung \ref{fig:auslastungTest1_gtm}}.
...
Die Auslastung aller \Gls{vm}s ist in Abbildung \ref{fig:auslastungTest1UMN} sowie \ref{fig:auslastungTest1UMN_gtm_umn} dargestellt.
...
Trotz der Nutzung von vier Prozessorkerne ist die Auslastung der CPU durch die \Gls{umn} Instanz sehr hoch.
...
11 & 4226 & 110 \\ \hline
\end{tabular}
\caption[Testergebnis Lasttest der Aggregation mit PostgreSQL VM]{Testergebnis Lasttest der Aggregation mit PostgreSQL VM}
...
\caption[Auslastung der PostgreSQL VM im Lasttest der Aggregatoin]{Auslastung der PostgreSQL VM im Lasttest der Aggregation, 13:09-13:32 Uhr}
...
11 & 2365 & 150 \\ \hline
\end{tabular}
\caption[Testergebnis Lasttest der Aggregation mit Kartendarstellung und PostgreSQL VM]{Testergebnis Lasttest der Aggregation mit Kartendarstellung und PostgreSQL VM}
...
Die Auslastung beträgt 165MB bis 270MB der Arbeitsspeichers und 450\%{} bis 680\%{} der CPU, siehe Abbildung \ref{fig:auslastungTest1UMN_pg}.
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{Testergebnisse/umn_aggregation_pg_Testlauf.png}
\caption[Auslastung der PostgreSQL VM im Lasttest der Aggregation mit Kartendarstellung]{Auslastung der PostgreSQL VM im Lasttest der Aggregation mit Kartendarstellung, 13:40-14:09 Uhr}
...
Zwar existieren bei jedem Knoten Leistungsspitzen bis zu 80\%{}, jedoch dauert eine solche Auslastung maximal fünf Minuten und das Auftreten dieser Spitzenwerte ist während der Testreihe unterschiedlich sowie nicht deckungsgleich zwischen den Knoten.
\begin{figure}
\centering
...
11 & 556751 & 24039 & 974 \\ \hline
\end{tabular}
\caption[Testergebnis Lasttest der Verarbeitung der PostgreSQL VM]{Testergebnis Lasttest der Verarbeitung der PostgreSQL VM}
\label{tbl:ergebnisseTest2_pg}
\end{table}

Die Auslastung der PostgreSQL \Gls{vm} ist ähnlich, wie in Abbildung \ref{fig:auslastungTest2_pg} dargestellt.
Dabei liegt die Auslastung der CPU zwischen 30\%{} und 62\%{}.
Pro Testdurchlauf wird etwa 150MB Arbeitsspeicher verwendet und bei Ende des Durchlaufes wieder freigegeben.
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{Testergebnisse/jdbc_contouring_pg_Testlauf.png}
\caption[Auslastung der PostgreSQL VM im Lasttest der Verarbeitung]{Auslastung der PostgreSQL VM im Lasttest der Verarbeitung, 10:07-13:09 Uhr}
...
Als Testsystem kam ein Postgres-XL Cluster mit zwölf DataNodes, sechs Coordinators, sechs GTM Proxys und einem GTM sowie eine PostgreSQL Installation zum Einsatz.
Die Ergebnisse zu den zwei Systemen werden gegenüber gestellt.
...
\end{tabular}
\caption[Vergleich der CPU Auslastung]{Vergleich der CPU Auslastung, GTM VM nicht berücksichtigt}
\label{tbl:verglcpu}
\end{table}
...
Die Mittlung fand je Graph statt. Bei Postgres-XL wurden aus sechs gemittelten Werten ein Durchschnittswert erzeugt.


Die Verarbeitungsleistung konnte bei Postgres-XL nur mit einem Coordinator gemessen werden.
Aus diesem Grund unterscheidet sich die Laufzeit eines Testlaufes nicht relevant zwischen den Systemen.
